...
function renderRecipes(){
  ...
  filtered.forEach(function(r,ri){
    ...
    var rs=r.parts.map(function(p){return COLORS.find(function(c){return c.name===p.name&&c.brand===p.brand;})||{hex:'#888888',name:p.name};});
    ...
    r.parts.forEach(function(p,i){
      ...
      partsHtml+='<div class="recipe-part">'
        ...
        +'<button class="rp-sub-btn" title="Find closest match in any brand" onclick="showRecipeSubstitute(this,'+ri+','+i+')">⇄</button>'
        +'</div>'
...
function showRecipeSubstitute(btn, recipeIdx, partIdx){
  ...
  var original = COLORS.find(function(c){return c.name===p.name&&c.brand===p.brand;});
  if(!original) return;
...
function dismissBanner(){
  localStorage.setItem('wc_banner_dismissed','1');
