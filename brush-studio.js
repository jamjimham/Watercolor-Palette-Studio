...
function populateChipModal(c, isReferenceMode){
  ...
  requestAnimationFrame(()=>{
    ...
    renderSwatchCanvas(hero, c.hex, c.gran, c.transparency||'ST', W, H);
  });

  // ── Dry shift ──
  const shift=estimateDryShift(c.transparency||'ST', !!c.gran);
  ...
  requestAnimationFrame(()=>{
    ...
    renderDilutionStrip(cv, c.hex, c.gran, c.transparency||'ST', W, H);
  });
}
...
function chipAddToPalette(){
  ...
  addColorToPalette(sel.value, _chipOpenColor.name, _chipOpenColor.brand);
  closeDilutionModal();
}
...
function addColorToPalette(palId,colorName,colorBrand){
  const pal=palettes.find(p=>p.id===palId);
  const col=COLORS.find(c=>c.name===colorName&&c.brand===colorBrand);
  if(!pal||!col) return;
  const already=pal.colors.some(c=>c.name===col.name&&c.brand===col.brand);
