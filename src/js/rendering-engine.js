...
function estimateDryShift(transparency, gran, medium){
  if(medium==='gouache'){
    // Gouache dries close to its wet appearance — a much smaller shift than
    // watercolor, since that shift is driven by a thin wash sinking into
    // paper as it dries, and gouache isn't applied that way.
    return {level:'minimal', pct:'~5–10%', desc:"Dries close to its wet appearance — gouache doesn't rely on a thin absorbed wash the way watercolor does, so the shift is much smaller."};
  }
  let score=0;
...
function renderSwatchCanvas(canvas,hex,gran,transparency,W,H,medium){
  ...
  if(medium==='gouache'){
    paintGouacheSwatchRegion(ctx, hex, PW, PH, dpr, 0, 0, 1.0);
  } else {
...
// Gouache renders opaque and matte rather than translucent — minimal paper
// bleed-through at full strength (unlike watercolor's inherent transparency),
// a flat, even surface rather than watercolor's wash/granulation-bloom
// model, and no glossy sheen highlight (gouache dries chalky-matte, not
// glassy). Diluting with water still reduces coverage and lets paper show
// through more — that part of the physical behavior genuinely carries over,
// and thinned-out gouache does start to behave more like a wash.
function paintGouacheSwatchRegion(ctx, hex, PW, PH, dpr, ox0, oy0, dilution){
  const[r,g,b]=parseHex(hex);
  const pR=250, pG=247, pB=242; // warm paper (matches --paper #faf7f2)
  let _hv=0; for(let _ci=0;_ci<hex.length;_ci++){ _hv=(_hv*31+hex.charCodeAt(_ci))&0xffffff; }
  const ox=ox0+(_hv%200), oy=oy0+((_hv>>8)%200);
  ...
  for(let py=0; py<PH; py++){
    ...
    for(let px=0; px<PW; px++){
      ...
      // Very subtle brush-texture variation — much gentler than watercolor's
      // wash/edge-pooling/gravity model, since gouache lays down flat and
      // even rather than pooling and settling as it dries.
      const texture = fbmAniso(ox+nx*0.10, oy+ny*0.10, 2, 1.2, 0.6);
      let baseCoverage = 0.97 + (texture-0.5)*0.06;
      baseCoverage = Math.max(0.85, Math.min(1.0, baseCoverage));

      // Coverage falls off with dilution — same physical idea as
      // watercolor's density curve, but starting from near-total opacity
      // instead of an inherently translucent wash.
      const loadCurve = Math.pow(dilution, 0.45);
      const cover = Math.max(0.04, baseCoverage * loadCurve);

      let outR = r*cover + pR*(1-cover);
      let outG = g*cover + pG*(1-cover);
      let outB = b*cover + pB*(1-cover);

      // Faint matte-surface jitter only — deliberately no sheen highlight
      // and no wet-bloom edge lightening, since those simulate a glossy
      // translucent wash that gouache just doesn't produce.
      const jitter = (texture-0.5) * 5;
      outR += jitter; outG += jitter; outB += jitter;
...
function renderDilutionStrip(canvas, hex, gran, transparency, W, H, medium){
  ...
  DILUTION_STOPS.forEach((stop,i)=>{
    ...
    if(medium==='gouache'){
      paintGouacheSwatchRegion(segCtx, hex, thisSegW, PH, dpr, i*137, i*59, stop.d);
    } else {
