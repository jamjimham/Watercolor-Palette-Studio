...
function renderHarmonyColorGrid() {
  ...
  grid.innerHTML = filtered.map(function(c){
    ...
    return '<div class="'+hcls+'" data-name="'+escapeHtml(c.name)+'" data-brand="'+c.brand+'" data-medium="'+(c.medium||'watercolor')+'" onclick="setHarmonyRoot(this.dataset.name,this.dataset.brand,this.dataset.medium)">'
      +'<div class="harmony-pick-swatch" style="background:'+c.hex+';"></div>'
      +'<div class="harmony-pick-name">'+escapeHtml(c.name)+'</div>'
      +'</div>';
  }).join('');
}

function setHarmonyRoot(name, brand, medium) {
  harmonyColor = COLORS.find(function(c){ return c.name===name&&c.brand===brand&&(c.medium||'watercolor')===(medium||'watercolor'); });
    ...
    html += '<div class="harmony-slot'+(slot.isRoot?' root':'')+'">'
      ...
      + '<div class="harmony-slot-info">'
        ...
        + '<div class="harmony-slot-actions">'
          + '<button class="sort-btn" onclick="shopAdd(\''+escJsAttr(c.name)+'\',\''+escJsAttr(c.brand)+'\',\''+(c.medium||'watercolor')+'\')">+ Shopping List</button>'
        + '</div>'
      + '</div>'
      + '</div>';
  });
