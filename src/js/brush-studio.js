...
function renderBrushTray(pal){
  ...
  tray.innerHTML=pal.colors.map((c,i)=>
    `<div class="brush-well${brushState.loadedIdx===i?' loaded':''}" style="background:${c.hex};" title="${escapeHtml(c.name)}" onclick="loadBrushColor(${i})"></div>`
  ).join('');
}
...
function updateBrushLoadedChip(pal){
  ...
  chip.innerHTML=c ? `<div class="brush-loaded-swatch" style="background:${c.hex};"></div> ${escapeHtml(c.name)}` : 'No paint loaded';
}
