...
function renderMixBarHtml(pal){
  ...
  const chipHtml=sel.map((c,i)=>`<div class="mix-chip" style="background:${c.hex};" title="${escapeHtml(c.name)}"></div>`).join('');
  return `<div class="mix-bar" id="mix-bar">
  ...
  cols.forEach((row,ri)=>{
    ...
    cols.forEach((col,ci)=>{
      ...
      if(ri===ci){
        ...
        html+=`<td style="width:${w};height:${w};"><div class="mix-cell${mud.muddy?' muddy':''}" style="background:${mixed};width:100%;height:100%;" title="${escapeHtml(row.name)} + ${escapeHtml(col.name)} = ${mixed.toUpperCase()}${mud.muddy?' (muddy — '+Math.round(mud.drop*100)+'% chroma loss)':''}">
