  ...
  if(!palettes.length){
    ...
    list.innerHTML=palettes.map(p=>{
      ...
      return`<div class="swipe-row" data-swipe-action="deletePalette(null,'${p.id}')">
        ...
        <span class="palette-item-name">${escapeHtml(p.name)}</span>
        <span class="palette-item-count">${p.colors.length}</span>
        <div class="palette-item-actions">
          <button class="palette-item-btn" title="Duplicate palette" aria-label="Duplicate palette" onclick="event.stopPropagation();duplicatePalette(event,'${p.id}')">⧉</button>
          <button class="palette-item-btn del" title="Delete palette" aria-label="Delete palette" onclick="event.stopPropagation();deletePalette(event,'${p.id}')">✕</button>
        </div>
      </div></div>`;
    }).join('');
    initSwipeRows(list);
  ...
  area.innerHTML=`
    <div class="palette-header">
      <select class="palette-mobile-switcher" id="palette-mobile-switcher" onchange="selectPalette(this.value)" aria-label="Switch palette">
        ${palettes.map(p=>`<option value="${p.id}"${p.id===pal.id?' selected':''}>${escapeHtml(p.name)} (${p.colors.length})</option>`).join('')}
      </select>
      <input class="palette-name-input" value="${escapeHtml(pal.name)}" onchange="renamePalette('${pal.id}',this.value)" maxlength="40">
...
function renderSwatchView(pal,content){
  ...
  function cardHtml(c){
    ...
    return `<div class="sg-card${c.custom?' sg-custom':''}${isSelected?' sg-selected-for-mix':''}" draggable="false" data-idx="${realIdx}" data-palid="${pal.id}"${clickToSelect}>
        ...
        <div class="sg-swatch"${mixModeActive?'':` onclick="openDilutionModal('${pal.id}',${realIdx})" title="Tap for full paint details"`} style="background:${c.hex};">
          ...
          ${!mixModeActive?`<button class="sg-del" aria-label="Remove from palette" onclick="event.stopPropagation();removeFromPalette('${pal.id}',${realIdx})">✕</button>
          <button class="sg-cam" id="sgcam_${pal.id}_${realIdx}" title="${hasPhoto?'Replace or remove photo':'Attach photo'}" aria-label="${hasPhoto?'Replace or remove photo':'Attach photo'}" onclick="event.stopPropagation();swatchCamAction('${pal.id}',${realIdx},${hasPhoto})">${hasPhoto?'📷✓':'📷'}</button>
        ...
        <div class="sg-body">
          <div class="sg-name">${escapeHtml(c.name)}</div>
          <div class="sg-pig">${c.custom?escapeHtml((c.mixedFrom||[]).join(' + ')):escapeHtml(c.pigment)}</div>
          <div class="sg-meta">
        ...
        ${!mixModeActive?`<div class="sg-note-wrap">
          <input class="sg-note-input" type="text" placeholder="Add a note…" value="${escapeHtml(note)}" 
  ...
  const io=new IntersectionObserver(entries=>{
    entries.forEach(e=>{
      ...
      queueRender(()=>renderSwatchCanvas(cv,c.hex,c.gran,c.transparency,W,H,c.medium));
      io.unobserve(cv);
    });
  },{rootMargin:'200px'});
